...
const generateMockData = (count = 10) => {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    cmpNm: `(주)3자단가업체${i + 1}`,
    cntrctNm: `2025년 3자단가 계약 ${i + 1}건`,
    cntrctDtlInfoUrl: '#',
    dminsttNm: `수요기관${i + 1}`,
    dminsttNmDetail: `지역${i + 1}`,
    prdctClsfcNo: `분류${i + 1}`,
    cntctCnclsMthdNm: '수의계약',
    ntceNo: `20250100${i + 1}`,
    firstCntrctDate: '2025-01-01',
    firstCntrctAmt: 3000000 * (i + 1),
    cntrctDate: '2025-01-15',
    thtmCntrctAmt: 3300000 * (i + 1),
    cntrctCnt: 1,
    checked: i % 2 === 0
  }));
};
...
const fetchData = async () => {
  ...
  // TODO: 나중에 이 부분은 axios.get('/api/data', { params: { category: 'shoppingMall', ...filters } })로 교체해야 함
  setTimeout(() => {
...
const toggleSave = (item) => {
  // TODO: 나중에 이 부분은 axios.post('/api/update-checked', { category: 'shoppingMall', id: item.id, checked: !item.checked })로 교체해야 함
