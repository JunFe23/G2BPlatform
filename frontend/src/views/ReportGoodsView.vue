...
const generateMockData = (count = 10) => {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    cmpNm: `(주)보고서업체${i + 1}`,
    cntrctNm: `2025년 보고서 물품 계약 ${i + 1}건`,
    cntrctDtlInfoUrl: '#',
    dminsttNm: `수요기관${i + 1}`,
    dminsttNmDetail: `지역${i + 1}`,
    prdctClsfcNo: `물품분류${i + 1}`,
    cntctCnclsMthdNm: '일반경쟁',
    ntceNo: `20250100${i + 1}`,
    firstCntrctDate: '2025-01-01',
    firstCntrctAmt: 1000000 * (i + 1),
    cntrctDate: '2025-01-15',
    thtmCntrctAmt: 1200000 * (i + 1),
    cntrctCnt: 1,
    checked: i % 3 === 0
  }));
};
...
const fetchData = async () => {
  ...
  // TODO: 나중에 이 부분은 axios.get('/api/data', { params: { category: 'report-goods', ...filters } })로 교체해야 함
  setTimeout(() => {
...
const toggleSave = (item) => {
  // TODO: 나중에 이 부분은 axios.post('/api/update-checked', { category: 'report-goods', id: item.id, checked: !item.checked })로 교체해야 함
