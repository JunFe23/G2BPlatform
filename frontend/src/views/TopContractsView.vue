...
const generateMockData = (count = 10) => {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    type: i % 3 === 0 ? '물품' : i % 3 === 1 ? '용역' : '공사',
    cmpNm: `(주)탑업체${i + 1}`,
    cntrctNm: `2025년 TOP 계약 ${i + 1}건`,
    cntrctDtlInfoUrl: '#',
    dminsttNm: `수요기관${i + 1}`,
    ...
    firstCntrctAmt: 4000000 * (i + 1),
    cntrctDate: '2025-01-15',
    thtmCntrctAmt: 4400000 * (i + 1),
    cntrctCnt: 1,
    checked: i % 2 === 0
  }));
};
...
const fetchData = async () => {
  ...
  // TODO: 나중에 이 부분은 axios.get('/api/data', { params: { category: 'onlyTop', ...filters } })로 교체해야 함
  setTimeout(() => {
...
const toggleSave = (item) => {
  // TODO: 나중에 이 부분은 axios.post('/api/update-checked', { category: 'onlyTop', id: item.id, checked: !item.checked })로 교체해야 함
