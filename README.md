...
- **보고서 데이터**  
  ...
  - **쇼핑몰(제3자단가)**: `shopping_mall_flat` — 보고서 화면은 `GET /api/report/shopping-mall`, 엑셀은 `GET /api/report/shopping-mall/excel` (레거시: `GET /api/shopping-mall/flat`)  
  ...
  - **대시보드** (`ReportDashboardView`):  
    ...
    - 시장현황 탭의 **연차계약 목록** 섹션은 제거됨(목록형 UI는 별도 보고서 화면에서 조회)
...
| 상위 메뉴 | 하위 |
|-----------|------|
| **보고서 데이터** | 물품, 쇼핑몰, 용역, 공사, 탑인더스트리·탑정보통신 수주현황(보고서), **수주대상 사업탐색**(보고서·API 연동 전 동일 UI), 대시보드 |
| **API 데이터** | 물품, 용역, 공사, 3자단가, 탑인더스트리·탑정보통신 수주현황(API), **수주대상 사업탐색** |
| **관리자** | 회원관리, CSV 업로드 |
...
| 쇼핑몰 flat 목록(레거시) | GET | `/api/shopping-mall/flat` |
