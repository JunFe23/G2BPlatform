...
## 39. G2B-28 — 사업탐색(with 설계&감리) 화면 흐름 목업 (2026-06-30~2026-07-02)
...
- 2026-07-01 클라이언트 답변 반영 목업 변경:
  - 커밋 `52f5ab0` (`G2B-28 feat: 사업탐색 추천분류 목업 보완`) 푸시 및 운영 web 반영 완료.
  - `영업대상(O/X)` 컬럼 제거.
...
- 2026-07-02 추가 클라이언트 답변 반영 목업 변경:
  - 상태: **로컬 구현/빌드 완료, 커밋·푸시·운영 web 반영 예정**.
  - 조회 화면에서 `입찰계약방법` 필터 제거.
  - 조회 화면 기간 검색은 기존 연도/월/기간 구조를 유지하고 실제 목업 필터에도 적용.
  - `수주대상 확정하기` 모달의 기간 조회를 연도/월/기간 방식으로 확장.
  - `대상 제외 추천리스트`도 다른 리스트와 동일하게 행별 체크박스 방식으로 변경:
    - 제외 추천 사유(`제외 키워드`)는 함께 표시.
    - 체크 시 확정 예정 항목으로 이동.
  - `대상 추천리스트`에만 전체 선택 체크박스 유지.
  - `미분류`와 `대상 제외 추천리스트`에는 전체 선택 없음.
  - 수기입력 라벨 `착수일자` → `착공일자`로 변경.
  - 수기입력에 `비고` textarea 추가.
- 요구/결정:
  ...
  - 2026-07-02 클라이언트 재요청 기준으로 라벨은 `착공일자`로 표시.
